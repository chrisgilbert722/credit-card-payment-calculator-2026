...
import { calculateCreditCardPayment } from './logic/creditCardPaymentCalculations';
import type { CreditCardPaymentInput } from './logic/creditCardPaymentCalculations';

function App() {
  const [values, setValues] = useState<CreditCardPaymentInput>({
    balance: 5000,
    apr: 22.99,
  ...
  const handleChange = (field: keyof CreditCardPaymentInput, value: number | boolean) => {
    setValues(prev => ({ ...prev, [field]: value as number }));
  };

  const result = calculateCreditCardPayment(values);
